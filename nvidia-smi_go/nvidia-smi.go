...
func main() {
	binPath := flag.String("bin", "c:/Program Files/NVIDIA Corporation/NVSMI/nvidia-smi.exe", "nvidia-smi full path")
	...
	metrics := "fan.speed,memory.total,memory.used,memory.free,power.draw,temperature.gpu,name,uuid,compute_mode,utilization.gpu,utilization.memory,index"
	...
	for i := 0; i < gpuCount; i++ {
		...
		fmt.Printf("power_draw=%s,", strings.TrimSpace(splitResults[4]))  // Watts
